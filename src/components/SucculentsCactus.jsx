...
import "./SucculentsCactus.css"; // reusing same CSS

const ITEMS_PER_PAGE = 12;

const SucculentsCactus = () => {
  const [plants, setPlants] = useState([]);
  const [currentPage, setCurrentPage] = useState(1);
  const [selectedPlant, setSelectedPlant] = useState(null);

  useEffect(() => {
    fetch("/db/SucculentsCactus.json")
      .then((res) => res.json())
      .then((data) => setPlants(data.SucculentsCactus))
  ...
  return (
    <div className="indoor-container">
      ...
      {/* Banner for Succulents */}
      <div className="banner succulents-banner"></div>
      ...
      {selectedPlant && (
        <div className="details-overlay" onClick={() => setSelectedPlant(null)}>
          <div className="details-panel" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="detail-sci">{selectedPlant.family}</p>
...
export default SucculentsCactus;
